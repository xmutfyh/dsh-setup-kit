...
| dsh-plugin-writing-guard | 1.2.0 | 确定性 manuscript integrity guard（Revision Integrity + claim-bound + alignment credibility）：Scholarship Lock + Epistemic Lock（因果/证据力+角色排除、hedge、claim-bound marker 守恒、scope 前缀附着、alignment-uncertain、版本差距保护、exec.token 自动前后对比）+ findingKind + SKILL.md | ✅ Config 兼容 cordis 4（`export const Config`→`const DEFAULT_CONFIG`） |
