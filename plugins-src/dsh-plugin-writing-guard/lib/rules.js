...
export function diffEpistemic(before, after) {
    const out = { claimDrift: [], negationRemoved: [], negationAdded: [], nullResultRemoved: [], scopeRemoved: [] };
    ...
    for (const { beforeIdx, afterIdx, sim } of pairs) {
        ...
        for (let i = 0; i < bSpans.length; i++) {
            ...
            if (j < 0 || j >= aSpans.length || aUsed.has(j))
                continue;
            aUsed.add(j);
    ...
    if (opts?.original !== undefined && opts.original.trim()) {
        ...
        for (const d of ed.claimDrift) {
            ...
            hits.push({
                ...
                message: `语言润色${up ? '抬高了' : '削弱了'}科学主张的${axis}（${d.axis === 'causal' ? '因果阶梯' : '证据阶梯'} ${d.levelBefore} → ${d.levelAfter}）。` +
