...
async function readCdpStreamToFile(streamHandle, sessionId, outFile, maxBytes = 209715200) {
  const absolute = path.resolve(outFile);
  const partial = `${absolute}.part-${process.pid}-${Date.now()}`;
  ...
  try {
    while (true) {
      const response = await sendCDP('IO.read', { handle: streamHandle, size: 1048576 }, sessionId);
      if (response.error) throw new Error(response.error.message || JSON.stringify(response.error));
      const result = response.result || {};
      const chunk = Buffer.from(result.data || '', result.base64Encoded ? 'base64' : 'utf8');
      if (chunk.length) {
        total += chunk.length;
        if (total > maxBytes) throw new Error(`resource_too_large:${total}`);
    ...
    fs.writeFileSync(partial, Buffer.concat(chunks, total));
    fs.renameSync(partial, absolute);
    completed = true;
    return {
      file: absolute,
      bytes: total,
      head: Array.from(head),
      signature: head.toString('ascii'),
    };
  } finally {
    await sendCDP('IO.close', { handle: streamHandle }, sessionId).catch(() => {});
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    // POST /fetchFile?target=xxx
    // body: JSON {"url":"https://...","out":"C:\\path\\paper.pdf","requirePdf":true,"maxBytes":209715200}
    // Uses Chrome's network stack with the current page credentials, bypassing renderer CORS
    // without exposing cookies or session storage to the caller.
    else if (pathname === '/fetchFile') {
      const sid = await ensureSession(q.target);
      let body;
      try {
        ...
        res.statusCode = 400;
        res.end(JSON.stringify({ ok: false, error: 'body 必须是 JSON' }));
        return;
      }

      const resourceUrl = String(body.url || '');
      const outFile = String(body.out || '');
      const requirePdf = body.requirePdf !== false;
      const maxBytes = Number(body.maxBytes || 209715200);
      if (!/^https?:\/\//i.test(resourceUrl) || !outFile) {
        res.statusCode = 400;
        res.end(JSON.stringify({ ok: false, error: '需要合法的 http(s) url 和 out 路径' }));
        return;
      }

      const frameTree = await sendCDP('Page.getFrameTree', {}, sid);
      if (frameTree.error) {
        res.end(JSON.stringify({ ok: false, error: frameTree.error.message || 'Page.getFrameTree failed' }));
        return;
      }
      const frameId = frameTree.result?.frameTree?.frame?.id;
      if (!frameId) {
        res.end(JSON.stringify({ ok: false, error: '无法取得页面 frameId' }));
        return;
      }

      const loaded = await sendCDP('Network.loadNetworkResource', {
        frameId,
        url: resourceUrl,
        options: {
          disableCache: Boolean(body.disableCache),
      ...
      if (loaded.error) {
        res.end(JSON.stringify({
          ok: false,
          error: `Network.loadNetworkResource: ${loaded.error.message || JSON.stringify(loaded.error)}`,
        }));
        return;
      }

      const resource = loaded.result?.resource || {};
      if (!resource.success || !resource.stream) {
        res.end(JSON.stringify({
          ok: false,
          error: resource.netErrorName || 'network_resource_load_failed',
          httpStatus: resource.httpStatusCode || 0,
      ...
      let saved;
      try {
        saved = await readCdpStreamToFile(resource.stream, sid, outFile, maxBytes);
      } catch (error) {
        res.end(JSON.stringify({
          ok: false,
      ...
      const isPdf = saved.signature.startsWith('%PDF-');
      if (requirePdf && !isPdf) {
        try { fs.unlinkSync(saved.file); } catch {}
        res.end(JSON.stringify({
          ok: false,
          error: 'not_a_pdf',
          httpStatus: resource.httpStatusCode || 0,
    ...
    // GET /info?target=xxx - 获取页面信息
      ...
      res.end(JSON.stringify({
        ...
        endpoints: {
          ...
          '/fetchFile?target=': 'POST JSON={url,out,requirePdf,maxBytes} - 用当前浏览器授权下载二进制文件',
