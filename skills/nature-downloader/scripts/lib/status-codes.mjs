...
export const STATUS = Object.freeze({
  ...
  FULINK_SEARCH_NO_MATCH: "fulink_search_no_match",
  FULINK_MATCH_WITHOUT_DELIVERY: "fulink_match_without_delivery",
  FULINK_LOGIN_REQUIRED: "fulink_login_required",
  FULINK_REQUEST_FORM: "fulink_request_form",
  FULINK_WAITING_DELIVERY: "fulink_waiting_delivery",
  FULINK_PDF_DOWNLOADED: "fulink_pdf_downloaded",
  FULINK_PDF_FETCH_FAILED: "fulink_pdf_fetch_failed",
  FULINK_FAILED: "fulink_failed",
...
const NO_SUBSCRIPTION_RE =
  /does not subscribe|not subscribed|purchase pdf|buy pdf|subscribe to this content|institution(?:al)? access unavailable|does not provide access to this content|you do not have access to this content|available to purchase|buy this article|pay per view access|institutional administrator|学校.*未订阅|无订阅|购买\s*pdf/i;

const DOWNLOAD_DISABLED_RE =
  /downloading and printing are disabled|download(?:ing)?\s+(?:and\s+)?printing\s+(?:is|are)\s+disabled/i;
...
export function classifyWall(url, title, bodyHint = "") {
  ...
  // 4. Explicit institution subscription / purchase wall.
  if (NO_SUBSCRIPTION_RE.test(s)) {
    return { status: STATUS.LIBRARY_NO_PERMISSION, reason: "institution does not subscribe / purchase required" };
  }

  // Some Wiley viewers expose readable HTML/PDF pages but explicitly disable
  // file download and printing. This is a publisher restriction, not a fetch error.
  if (DOWNLOAD_DISABLED_RE.test(s)) {
    return { status: STATUS.PUBLISHER_BLOCKED_WAITING_USER, reason: "publisher disables downloading and printing" };
  }
