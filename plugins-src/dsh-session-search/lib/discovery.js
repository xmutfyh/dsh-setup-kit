 ...
 */
export async function discoverFiles(enabled, roots = {}) {
        ...
        for (const path of await walkFiles(root, 3)) {
            if (path.endsWith('.jsonl')) {
                ...
                if (name.startsWith('agent-'))
                    continue; // subagent transcripts are part of the parent
    ...
    if (enabled.has('opencode')) {
        ...
        try {
            ...
            if (st.isFile())
                files.push({ source: 'opencode', path: dbPath, sessionId: 'opencode-db' });
        }
        catch {
