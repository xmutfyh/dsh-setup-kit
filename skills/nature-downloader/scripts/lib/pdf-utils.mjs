...
import { evalJs, proxyPostJson } from "./cdp-utils.mjs";
 ...
 */
async function fetchDirectToFile(
  proxy,
  target,
  url,
  outPath,
  ...
  try {
    const result = await proxyPostJson(
      proxy,
      "/fetchFile",
      {
        url,
    ...
    if (!result || !result.ok) {
      return {
        ok: false,
        err: result?.error || "cdp_network_fetch_failed",
    ...
    return {
      ok: true,
      file: result.file || path.resolve(outPath),
      bytes: result.bytes,
      contentType: result.contentType || "",
      status: result.httpStatus,
      direct: true,
    ...
    if (/HTTP 404|未知端点|unknown endpoint/i.test(message)) {
      return { ok: false, err: "cdp_fetch_endpoint_unavailable", supported: false };
    }
    return { ok: false, err: `cdp_network_fetch_error:${message.slice(0, 300)}`, supported: true };
  }
}
...
export async function fetchToFile(proxy, target, url, outPath, { onProgress, maxBytes } = {}) {
  const direct = await fetchDirectToFile(proxy, target, url, outPath, {
    requirePdf: true,
    maxBytes,
  });
  ...
  if (!meta.ok) {
    return {
      ok: false,
      err: `${direct.err}; page_fetch:${meta.err}`,
      status: direct.status,
      contentType: direct.contentType,
      head: direct.head || meta.head,
    };
  }
  ...
  return { ok: true, ...res, direct: false };
}
...
export async function fetchAnyToFile(proxy, target, url, outPath, { onProgress, maxBytes } = {}) {
  ...
  if (!meta.ok) return { ok: false, err: `${direct.err}; page_fetch:${meta.err}` };
  const res = await streamToDisk(
    proxy,
    target,
    meta.varName,
  ...
  return { ok: true, bytes: res.bytes, direct: false };
}
