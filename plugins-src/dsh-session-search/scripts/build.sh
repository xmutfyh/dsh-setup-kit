#!/bin/bash
# Build the dsh-session-search external plugin: compile src/ → lib/ (JS) and
# lib/types/ (declarations) with the dsh checkout's TypeScript.
#
# Dependency resolution mirrors marisa-panel and session-chatlog: the plugin's
# node_modules holds symlinks into the dsh checkout, so tsc type-checks against
# the same vendored/workspace packages the running dsh ships. Requires `dsh` on
# PATH (or DSH_CHECKOUT pointing at a checkout root).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

# Resolve the dsh checkout: DSH_CHECKOUT env wins, else dsh → bin/dsh → root.
