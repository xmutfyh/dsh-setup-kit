import { createToastTimer } from "./toast-timer.js";
const TOAST_ID = 'dsh-file-drop-toast';
const AUTO_DISMISS_MS = 8000;
export function createFileDropToast() {
    ...
    const closeToast = () => { timer.cancel(); hide(); };
    close.addEventListener('click', closeToast);
    root.addEventListener('mouseenter', () => { timer.pause(); });
    root.addEventListener('mouseleave', () => { if (root.style.display !== 'none')
        timer.resume(); });
    return {
        showError(text) {
