...
## `POST /fetchFile?target=<targetId>`

Uses the controlled Chrome tab's network stack and current authenticated session to fetch a resource directly to a local file. This avoids renderer CORS failures while keeping cookies and storage inside the browser.
...
```json
{
  ...
  "out": "C:\\work\\paper.pdf",
  "requirePdf": true,
  "maxBytes": 209715200
}
```

The response includes `ok`, `file`, `bytes`, `contentType`, `httpStatus`, and PDF signature validation. It does not bypass logins, paywalls, CAPTCHA, or publisher authorization.
