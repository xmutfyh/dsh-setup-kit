...
export function suppressImageFormatToast() {
    ...
    const suppress = (alert) => {
        ...
        if (alert.classList.contains(SUPPRESS_CLASS))
            return;
        const text = alert.textContent ?? '';
        if (!MATCH_PATTERNS.some(pattern => pattern.test(text)))
            return;
        ...
        for (const span of alert.querySelectorAll('span')) {
            ...
            if (own !== undefined && own !== '')
                span.classList.add('dsh-file-drop-original');
        }
    };
        ...
        for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
                ...
                if (node.matches(ALERT_SELECTOR))
                    ...
                    for (const alert of node.querySelectorAll(ALERT_SELECTOR))
                        suppress(alert);
                }
            }
        }
