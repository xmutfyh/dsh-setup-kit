...
| dsh-plugin-writing-guard | 1.2.3 | 确定性 manuscript integrity guard（Revision Integrity + claim-bound + claim identity）：Scholarship Lock（全方向含新增）+ Epistemic Lock（因果/证据力+角色排除、hedge、claim-bound 守恒、claimAnchor 指纹、scope 统一分类、alignment-uncertain、版本差距保护+不配对全局清单、exec.token 自动前后对比、event-level 指纹、invariant 恒显）+ findingKind + SKILL.md | ✅ Config 兼容 cordis 4（`export const Config`→`const DEFAULT_CONFIG`） |
