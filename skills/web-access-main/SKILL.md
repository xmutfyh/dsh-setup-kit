---
name: web-access-main
license: MIT
github: https://github.com/eze-is/web-access
description:
  需要真实浏览器环境的联网任务通过此 skill 处理：搜索、网页抓取、登录后操作、网页交互、社交媒体内容（小红书、微博、推特等）、动态渲染页面等。
  例外：软件/工具/包的安装与下载等命令行操作（pip/conda/uv/npm/pnpm/winget/git/curl 等）直接在会话中执行，不需要此 skill。
...
## 安装类任务例外（重要，优先于本 skill 其余规则）

软件、工具、包、依赖的**安装/更新/下载**必须在对话中直接通过命令行完成，
**不要**启动浏览器 CDP，也不要走本 skill 的浏览器流程。例如：

- `pip install` / `conda install` / `uv add` / `uv pip install`
- `npm install` / `pnpm add` / `winget install` / `choco install`
- `git clone` / `curl -O` / `wget` / `Invoke-WebRequest` 下载文件
- 配置命令（`npm config set`、`uv venv` 等）

仅当用户**明确要求**"用浏览器下载/在网页上操作"时才进入浏览器流程。
