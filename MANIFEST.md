...
| dsh-plugin-writing-guard | 0.9.1 | 确定性 manuscript integrity guard：Scholarship/Epistemic Lock（双轴因果力/证据力、子句级多主张、否定/零结果/scope 守恒、exec.token 自动前后对比）+ findingKind 性质标签 + SKILL.md | ✅ Config 兼容 cordis 4（`export const Config`→`const DEFAULT_CONFIG`） |
