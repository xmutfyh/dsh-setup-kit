...
| dsh-plugin-writing-guard | 1.1.0 | 确定性 manuscript integrity guard（Revision Integrity 完整 + claim-bound）：Scholarship Lock + Epistemic Lock（因果/证据力+角色排除、hedge、否定/零结果/scope/证据状态 claim-bound 守恒、版本差距保护、exec.token 自动前后对比）+ findingKind + SKILL.md | ✅ Config 兼容 cordis 4（`export const Config`→`const DEFAULT_CONFIG`） |
